...
// ── Edit Modal ────────────────────────────────────────────────────────────────
const EditModal = ({ category, onClose, onUpdated }) => {
  ...
  const handleSave = async () => {
    ...
    setErr('');
    try {
      await axios.patch(`${BASE}/api/editCategory/${category._id}`, { name });
      onUpdated();
      onClose();
    } catch (e) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <div className="relative w-full max-w-sm bg-gray-800 border border-violet-500/40 rounded-2xl shadow-2xl p-6 space-y-5">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-bold text-violet-400 uppercase tracking-widest">Edit Category</h3>
...
const DeleteModal = ({ category, onClose, onDeleted }) => {
  ...
  const handleDelete = async () => {
    setLoading(true);
    try {
      await axios.delete(`${BASE}/api/deleteCategory/${category._id}`);
      onDeleted();
      onClose();
    } catch {
      onClose();
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <div className="relative w-full max-w-sm bg-gray-800 border border-red-500/40 rounded-2xl shadow-2xl p-6 space-y-5">
        ...
        <p className="text-sm text-gray-400">
          <span className="text-white font-medium">"{category.name}"</span> Are you sure you want to delete this?
        </p>
...
const ViewCategories = () => {
  ...
  const fetchCategories = async () => {
    setLoading(true);
    try {
      const res = await axios.get(`${BASE}/api/getAllCategories`);
    ...
    <div className="w-full">
      ...
      <div className="flex items-center justify-between mb-4">
        <p className="text-gray-400 text-xs">
          Total: <span className="text-violet-400 font-semibold">{categories.length}</span> categories
        </p>
        <button
      ...
      {loading ? (
        ...
        <div className="text-center py-12 text-gray-600 text-sm">Catagory not found!</div>
      ) : (
                    ...
                    <div className="flex items-center justify-end gap-2">
                      <button
                        ...
                        className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium
                          ...
                          hover:bg-violet-500/20 hover:border-violet-500/50 transition-all duration-150"
                      >
      ...
      {editTarget && (
        <EditModal
          ...
          onUpdated={() => { fetchCategories(); showToast('Category updated!'); }}
        />
      )}
      {delTarget && (
        <DeleteModal
          ...
          onDeleted={() => { fetchCategories(); showToast('Category deleted!'); }}
        />
      )}
    </div>
  );
